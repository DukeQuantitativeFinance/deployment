...
from .app_customer_flow.customer_flow_base import Game

customer_flow_servers = {}

# route for user to submit file
@discrete_exchange_routes.route('/customer-flow/submit', methods = ['POST'])
@cross_origin()
def customer_flow_submit_trader():
    global customer_flow_servers
    ...
    if request.method == 'POST':
        ...
        customer_flow_servers[user_id] = Game()
        file = request.files['file[]']
        filename = file.filename
        print("filename: ", filename)
        
        # save file to this directory and also save a copy to submitted_traders directory
        filepath = os.path.join(os.path.dirname(__file__) + '/app_customer_flow', filename)
        file.save(filepath)
        shutil.copyfile(filepath, f'{os.path.dirname(__file__)}/customer-flow-submissions/{filename}')

        customer_flow_servers[user_id].import_individual_trader(filename)
        customer_flow_servers[user_id].runGame()
    return '<h1></h1>'

@discrete_exchange_routes.route('/customer-flow/results', methods = ['POST'])
@cross_origin()
def customer_flow_get_results():
    global customer_flow_servers
    ...
    if request.method == 'POST':
        ...
        position = customer_flow_servers[user_id].player_position
        balance = customer_flow_servers[user_id].player_balance
        net_worth = balance + sum(p * customer_flow_servers[user_id].market_mean for p in position)
        response = {'position': position, 'balance': balance, 'netWorth': net_worth}
        return jsonify(response)
